...
#if  !USE_SPI_FLASH_CHARATER_CODE_TABLE
const WCHAR uni2oem[] = {
...
#if USE_SPI_FLASH_CHARATER_CODE_TABLE      
WCHAR Read_Charater_Code_Table(UINT dir,int Addr)
{	
	WCHAR Ret;
	int TableBasePage=dir? OEM2UNI_TABLE_BASE : UNI2OEM_TABLE_BASE;
	Addr<<=1;

	Q_SpiFlashSync(FlashRead,TableBasePage*SPI_FLASH_PAGE_SIZE+Addr,sizeof(WCHAR),(u8 *)&Ret);
	return Ret;
}
#endif
...
)
{
	...
	if (src < 0x80) {	/* ASCII */
...
#if USE_SPI_FLASH_CHARATER_CODE_TABLE
		hi=CHARATER_CODE_TABLE_SIZE/4-1;
#else
		...
		for (n = 16; n; n--) {
			i = li + (hi - li) / 2;
#if USE_SPI_FLASH_CHARATER_CODE_TABLE
			tmp=Read_Charater_Code_Table(dir,i * 2);
			if (src == tmp) break;
			if (src > tmp)
#else
			if (src == p[i * 2]) break;
...
#if USE_SPI_FLASH_CHARATER_CODE_TABLE
		c = n ? Read_Charater_Code_Table(dir,i * 2 + 1) : 0;
#else
		c = n ? p[i * 2 + 1] : 0;
#endif
	}
