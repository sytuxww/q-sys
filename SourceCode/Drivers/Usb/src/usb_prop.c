...
/* Private functions ---------------------------------------------------------*/
static void DisplayUsbRequest(const char *Str)
{
	Debug("%s:",Str);
	Debug("Req:%02x %02x %04x %04x %04x\n\r",pInformation->USBbmRequestType,pInformation->USBbRequest,
		pInformation->USBwValues.w,pInformation->USBwIndexs.w,pInformation->USBwLengths.w);
}
...
{
	DisplayUsbRequest(__FUNCTION__);
