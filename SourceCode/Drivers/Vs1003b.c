...
#define VsDelay OS_TaskDelayMs

#if 0
static const u8 atab[943] = { /* Register addresses */
...
static const u16 DataTab[943] = { //vs的指令表
...
u16 VsGetHeadInfo(void)
{
    ...
    HEAD0=VsReadReg(REG_HDAT0); 
    HEAD1=VsReadReg(REG_HDAT1);
...
{
	VsWriteReg(REG_DECODE_TIME,0x0000);
	VsWriteReg(REG_DECODE_TIME,0x0000);//操作两次
}

//得到mp3的播放时间n sec
u16 VsGetDecodeTime(void)
{ 
    return VsReadReg(REG_DECODE_TIME);   
} 
...
void VsLoadPatch(void)
{
	...
	for (i=0;i<sizeof(DataTab);i++)
	{
		switch(i)
		{
			...
			case 798:
			case 927:
				VsWriteReg(7,DataTab[i]); 
				break;
			case 942:
				VsWriteReg(10,DataTab[i]); 
				break;
			default:
				VsWriteReg(6,DataTab[i]); 
		}		
	}
...
void VsGetSpec(u16 *p)
{
	...
	VsWriteReg(REG_WRAMADDR,0x1804);                                                                                             
	for (i=0;i<14;i++) 
	{                                                                               
		*p++=VsReadReg(REG_WRAM);//&0x63;//取小于100的数    
	} 
}
...
{ 
	VsWriteReg(REG_WRAMADDR,VS_GPIO_DDR); 
	VsWriteReg(REG_WRAM,ucDDRValue); 
} 

//设置io电平
void VsGpioWriteData(u8 ucWriteValue) 
{ 
	VsWriteReg(REG_WRAMADDR,VS_GPIO_OData); 
	VsWriteReg(REG_WRAM,ucWriteValue); 
} 
...
{ 
	...
	VsWriteReg(REG_WRAMADDR,VS_GPIO_IData); 
	ucReadValue = VsReadReg(REG_WRAM); 
	return(ucReadValue);
} 
...
{ 
	VsWriteReg(REG_VOL,((left<<8)|right));
}

//软复位
void VsSoftReset(void)
{
	VsWriteReg(REG_MODE, SM_RESET); //软件复位
	VsDelay(10);
    while(VS_DREQ==0);  //等待软件复位结束
    
    VsWriteReg(REG_MODE, SM_SDINEW); 
	//VsWriteReg(REG_CLOCKF, 0x8800);//设置vs1003的时钟,3倍频
	//VsWriteReg (REG_AUDATA, 0xBB81); //采样率48k，立体声
	//VsWriteReg(REG_BASS, 0);//设置重音
	//VsSetVolume(0x40,0x40);//设置音量
	//VsDelay(10);
    	
    //向vs1003发送4个字节无效数据，用以启动SPI发送
    VS_XDCS_SET(0);     //xDCS = 0
	VS_Write_Byte(0);
	VS_Write_Byte(0);
	VS_Write_Byte(0);
	VS_Write_Byte(0);
...
void VsLedSet(bool LedIO2,bool LedIO3)
{
	...
	if(LedIO2) IO_Map|=0x04;
	if(LedIO3) IO_Map|=0x08;
	VsGpioWriteData(IO_Map);//点亮io2和io3
	//while(VS_DREQ==0);
}
...
void VsStart(void)
{	
	VS_XRST_SET(0);//xReset = 0   复位vs1003();
	VS_CS_SET(1);
	VS_XDCS_SET(1);
	VS_XRST_SET(1);
	VsDelay(10); 
